...
class TestEvaluator(unittest.TestCase):
    ...
    def test_royal_flush_rank_after_turn(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('Kh'), treys.Card.new('Ah')],
            board=[treys.Card.new('Qh'), treys.Card.new('Jh'), treys.Card.new('Th'), treys.Card.new('2h')],
            expected_rank=9
        )
    ...
    def test_high_card_rank_after_turn(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('3d'), treys.Card.new('Jc')],
            board=[treys.Card.new('8s'), treys.Card.new('4h'), treys.Card.new('2s'), treys.Card.new('9c')],
    ...
    def test_royal_flush_rank_showdown(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('Kh'), treys.Card.new('Ah')],
            board=[treys.Card.new('Qh'), treys.Card.new('Jh'), treys.Card.new('Th'), treys.Card.new('2h'), treys.Card.new('7s')],
            expected_rank=9
        )
    ...
    def test_high_card_rank_showdown(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('3d'), treys.Card.new('Jc')],
            board=[treys.Card.new('8s'), treys.Card.new('4h'), treys.Card.new('2s'), treys.Card.new('9c'), treys.Card.new('Kd')],
            expected_rank=1
        )

    def test_weakest_hand_rank_during_preflop(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('2d'), treys.Card.new('7c')],
            board=[],
            expected_rank=0
        )
    ...
    def test_strongest_hand_rank_during_preflop(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('Ad'), treys.Card.new('Ac')],
            board=[],
            expected_rank=9
        )
    ...
    def test_single_suit_hand_rank_during_preflop(self):
        self._assert_rank_with_board(
            hand=[treys.Card.new('5h'), treys.Card.new('7h')],
            board=[],
            expected_rank=2
        )
